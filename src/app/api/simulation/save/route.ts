...
export async function POST(req: NextRequest) {
    try {
        ...
        const { projectId, history, report, warnings, settings } = body;

        if (!projectId || !history) {
            return NextResponse.json({ error: "Missing project ID or history data" }, { status: 400 });
        }

        // 1. Execute everything in a single transaction
        const result = await db.transaction(async (tx) => {
            ...
            // B. If runs exist, delete their results and the runs themselves
            if (runIds.length > 0) {
                await tx.delete(simulationResults)
                    .where(inArray(simulationResults.runId, runIds));

                await tx.delete(simulationRuns)
                    .where(inArray(simulationRuns.id, runIds));
            }

            // C. Create a NEW Simulation Run Record
            const [run] = await tx
                ...
                .values({
                    ...
                    report: report || "",
                    warnings: warnings || [],
                    inputData: settings || {}, // Store input metadata
                })
            ...
            // F. Batch Insert inside the transaction
            if (rowsToInsert.length > 0) {
                const chunkSize = 500;
                for (let i = 0; i < rowsToInsert.length; i += chunkSize) {
                    await tx.insert(simulationResults).values(rowsToInsert.slice(i, i + chunkSize));
                }
            }
