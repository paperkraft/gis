...
export function SetupView() {
    ...
    const handleRun = async () => {
        ...
        updateSettings({
            ...
            hydraulicStep: formData.hydraulicStep,

            units: formData.units,
            headloss: formData.headloss,
    ...
    return (
        <div className="flex flex-col h-full bg-white text-slate-700 animate-in slide-in-from-left-4">
            <div className="flex items-center gap-2 p-3 border-b border-slate-100 bg-slate-50">
                ...
                <div>
                    <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700">Simulation Setup</h2>
            ...
            <div className="p-3 border-t border-slate-100 bg-slate-50">
                <button 
                    ...
                    className={cn('w-full py-2.5 rounded shadow-sm flex items-center justify-center gap-2 text-xs font-bold text-white transition-all',
                         isSimulating ? 'bg-slate-400 cursor-wait' : 'bg-green-600 hover:bg-green-700'
                    )}>
                   {isSimulating ? (
                        <>
